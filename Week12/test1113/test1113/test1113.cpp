...
int main(void)
{
	...
	while (request != END)
	{
		...
		while (!inClientFile.eof()) // EOF만나면 operation 진행되지 않음
		{
			// display record
		...
		inClientFile.clear(); // reset eof for next input // 초기상태로 리셋
		inClientFile.seekg(0); // reposition to beginning of file // 처음부분으로 커서를 이동시킴, ios::beg가 생략됨(초기값이기 떄문)
